const int LED = 9;  //9번 핀을 사용하는 LED상수 정의
const int BUTTON = 2;  //버튼을 2번핀에 지정
...
void loop(){
  ...
  if (lastButton == LOW && currentButton == HIGH){ //버튼을 누름
    ledOn = !ledOn;  //LED 점멸 상태를 나타내는 ledOn 변수 값을 바꿈
  }
  lastButton = currentButton;  //이전 버튼값을 현재 버튼 값으로 변경
